...
export const userLoginRequest = (userData, history) => dispatch => {
...
export const getUserAccDetails = () => dispatch => {
  axios.get('/api/v1/user/useraccountdetails')
  .then((response) => {
...
export const adminLoginRequest = (userData, history) => dispatch => {
  axios.post('/api/v1/admin/signin', userData)
  .then((response) => {
    dispatch(adminLoginSuccess(response));
    const { token } = response.data;
    localStorage.setItem('adminAccessToken', token);
    setAuthToken(token);
    dispatch(setCurrentUser(jwt.decode(token), 'admin'));
    swal({
      title: "Hi there!",
      text: response.data.message,
      icon: "success"
...
export const adminSignupRequest = (userData, history) => dispatch => axios.post('/api/v1/admin/signup', userData)
  .then((response) => {
...
export const updateClientInfo = (id, updatedInfo, history) => dispatch => {
  // return console.log(id , 'iddddddd');
...
export const deposit = (id, depositData, history) => dispatch => {
